...
interface EventTransactionState {
  ...
  setFilters: (filters: Partial<EventTransactionFilters>) => void;
...
const initialFilters: EventTransactionFilters = {
  status: 'All',
  giftStatus: 'All',
  paymentMethod: 'All',
  searchTerm: '',
...
export const useEventTransactionStore = create<EventTransactionState>((set, get) => ({
  ...
  fetchEventTransactions: async (eventId: string, page = 1, searchTerm = '', sortBy = 'createdAt', sortOrder = 'desc') => {
    ...
    try {
      ...
      set({ error: err.response?.data?.message || 'Failed to fetch event transactions' });
